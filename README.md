...
⚠ **If you use a local image with `imagePullPolicy: Never` or `IfNotPresent`, make sure to add the `--docker` flag when installing K3s. Otherwise, you may encounter `ErrImagePull` or `ImagePullBackOff` errors when creating the pod.**
...
```bash
helm install <project-name> <path/to/helm/folder> -f <path/to/values.yaml>
```
...
✅ **Note:** Namespaces are automatically created by Helm.
